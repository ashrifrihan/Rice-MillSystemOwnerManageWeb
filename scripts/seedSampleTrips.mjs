 ...
 * seedSampleTrips.mjs
 * 
 * Seed complete sample trips to Firebase Realtime Database for testing
 * the Transport GPS page with realistic data.
 * 
 * Requires: Firebase Admin SDK credentials
 * Usage:
 *   node scripts/seedSampleTrips.mjs
 */
...
const credPath = process.env.FIREBASE_ADMIN_CREDENTIALS || './serviceAccountKey.json';
if (!fs.existsSync(credPath)) {
  console.error('❌ Service account JSON not found at', credPath);
  console.error('Set FIREBASE_ADMIN_CREDENTIALS env var or place serviceAccountKey.json in project root.');
  process.exit(1);
}
...
// Sample trips with complete route info
const sampleTrips = [
  {
    id: 'sample-trip-001',
...
// Live locations for active trips
const liveLocations = {
  'sample-trip-001': {
    lat: 7.4654,
    lng: 80.3658,
...
async function seedTrips() {
  try {
    console.log('🌱 Seeding sample trips to Firebase...\n');

    // Seed trips
    console.log('📦 Seeding trips...');
    for (const trip of sampleTrips) {
      await db.ref(`trips/${trip.id}`).set(trip);
      console.log(`   ✓ Trip ${trip.id} seeded`);
    }

    // Seed live locations
    console.log('\n🗺️  Seeding live locations...');
    for (const [tripId, location] of Object.entries(liveLocations)) {
      await db.ref(`liveLocations/${tripId}`).set(location);
      console.log(`   ✓ Live location for ${tripId} seeded`);
    }

    console.log('\n✅ All sample trips and live locations seeded successfully!');
    console.log('\nTrips IDs:');
    sampleTrips.forEach(trip => {
      console.log(`   - ${trip.id} (${trip.status})`);
    });
    console.log('\n💡 Tip: Refresh the Transport GPS page and you should now see these trips displayed.');
    console.log('         If you see "Calculating..." on route metrics, wait a moment for Google Maps to calculate.');
