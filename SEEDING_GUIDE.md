/**
 * Manual Firebase Seeding Guide for Transport GPS Sample Data
 * 
 * If admin credentials are not available, use the Firebase Console directly:
 * 1. Go to: https://console.firebase.google.com/project/ricemill-lk/database/data
 * 2. Navigate to Realtime Database
 * 3. Click "+" next to "trips" and add these entries
 * 
 * OR use this JavaScript in the browser console (while logged in as owner):
 */
 ...
 * ========================================
 * 1. Open Firebase Console: https://console.firebase.google.com/project/ricemill-lk/database/data
 * 2. Click Realtime Database
 * 3. Manually add the trips and liveLocations from sampleTrips and liveLocations above
 * 
 * Option 2: Use this code in Browser Console (while on app and logged in as owner)
 * ========================================
 * Copy and paste this code into the browser console:
 * 
 * import { ref, set } from 'firebase/database';
 * import { rtdb } from './src/firebase/config.jsx';
 * 
 * async function seedData() {
 *   try {
 *     console.log('🌱 Seeding trips...');
 *     for (const [id, trip] of Object.entries(sampleTrips)) {
 *       await set(ref(rtdb, `trips/${id}`), trip);
 *       console.log(`✓ Seeded ${id}`);
 *     }
 *     
 *     console.log('🗺️  Seeding live locations...');
 *     for (const [id, loc] of Object.entries(liveLocations)) {
 *       await set(ref(rtdb, `liveLocations/${id}`), loc);
 *       console.log(`✓ Seeded location ${id}`);
 *     }
 *     
 *     console.log('✅ Done! Refresh the page to see trips.');
 *   } catch (e) {
 ...
 * seedData();
 * 
 * Option 3: Provide Firebase Admin Credentials
 * ========================================
 * Place serviceAccountKey.json in the project root (do NOT commit to git)
 * Then run: node scripts/seedSampleTrips.mjs
 */

export { sampleTrips, liveLocations };
