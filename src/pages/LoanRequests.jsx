...
// Fallback mock data for loan requests
...
export function LoanRequests() {
  ...
  // Firebase Listener - Load loan requests
  useEffect(() => {
    if (!userEmail) return;
    
    setLoading(true);
    const loanRequestsRef = ref(db, 'loanRequests');
    
    const unsubscribe = onValue(loanRequestsRef, (snapshot) => {
      const requestsList = filterSnapshotByOwner(snapshot, userEmail);
      const fallback = mockLoanRequests.map(r => ({ ...r, owner_email: userEmail }));
      setLoanRequests(requestsList.length ? requestsList : fallback);
      setLoading(false);
    }, (error) => {
  ...
  // Original mock data removed - now using Firebase
  const mockData_removed = [
    {
      id: 'LR-001',
      dealerId: 'D-001',
  ...
  const calculateKPIs = () => {
    ...
    const totalRequestedAmount = loanRequests.reduce((sum, r) => sum + (r?.requestedAmount || 0), 0);
    const pendingAmount = pendingRequests.reduce((sum, r) => sum + (r?.requestedAmount || 0), 0);
    const approvedAmount = approvedRequests.reduce((sum, r) => sum + (r?.approvedAmount || r?.requestedAmount || 0), 0);
  ...
  const confirmAction = async () => {
    ...
    try {
      const loanRequestRef = ref(db, `loanRequests/${selectedRequest.id}`);
      const userName = userProfile?.name || user?.email || 'Manager';
      
      if (actionType === 'accept') {
        const approvedAmount = Math.round(selectedRequest.requestedAmount * 0.9);
        
        await update(loanRequestRef, {
          status: 'Approved',
        ...
        // Create approved loan in loanGiven collection
        const loanGivenRef = push(ref(db, 'loanGiven'));
        await set(loanGivenRef, {
          ...selectedRequest,
          loanId: loanGivenRef.key,
